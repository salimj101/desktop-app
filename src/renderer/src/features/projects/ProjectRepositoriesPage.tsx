...
export default function ProjectRepositoriesPage({ 
  ...
  // Default repository limit per project (can be made configurable later)
  const REPOSITORY_LIMIT = 10
  const hasReachedLimit = repositories.length >= REPOSITORY_LIMIT

  const fetchRepositories = async () => {
    try {
      setIsLoading(true)
      setError(null)
      // Use the existing getRepositoriesView API and filter by projectId
      const result = await window.api.getRepositoriesView()
      if (result && result.success) {
        // Filter repositories by projectId
        const allRepos = result.repositories || []
        const projectRepos = allRepos.filter((repo: any) => 
          repo.projectId === projectId
        )
        
        // Map to RepositoryStatus format
        const repos: RepositoryStatus[] = projectRepos.map((repo: any) => ({
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${
      ...
      <main className="flex-1 p-6">
        ...
        <button
          onClick={onBack}
          className={`flex items-center space-x-2 mb-6 text-sm transition-colors duration-300 ${
        ...
        {hasReachedLimit && (
          <div className={`p-3 mb-4 text-sm rounded-md ${
            ...
            This project has reached its repository limit of {REPOSITORY_LIMIT}. You cannot add more repositories until some are removed.
          </div>
        )}
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            <p className={`text-sm transition-colors duration-300 ${
              ...
              {repositories.length} repository{repositories.length !== 1 ? 's' : ''} in this project
              {hasReachedLimit && (
          ...
          <div className="flex space-x-3">
            ...
            <button
              ...
              disabled={isOffline || hasReachedLimit}
              style={isOffline || hasReachedLimit ? { opacity: 0.6, pointerEvents: 'none' } : {}}
              className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-300 disabled:bg-gray-400 disabled:cursor-not-allowed"
              title={hasReachedLimit ? `Repository limit of ${REPOSITORY_LIMIT} reached` : 'Add new repository to this project'}
            >
          ...
          {isLoading ? (
            ...
            <div className={`text-center p-8 rounded-lg ${isDark ? 'bg-gray-800' : 'bg-gray-100'}`}>
              ...
              <p className={`mt-2 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                This project doesn't have any repositories yet. Click 'Add Repository' to get started.
              </p>
            </div>
          )}
        </div>
      ...
      {isRegisterModalOpen && (
        <RegisterRepoForm
          ...
          defaultProjectId={projectId} // Pre-select this project
        />
      )}
      {selectedRepo && (
        <RepoDetailModal
