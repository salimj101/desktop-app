...
// --- ONE-TIME MIGRATION: Backfill developerId for all repos where missing ---
try {
  const session = getSession()
  if (session && session.userId) {
    const res = db
      .prepare(
        `UPDATE repositories SET developerId = ? WHERE developerId IS NULL OR developerId = ''`
      )
      .run(session.userId)
    if (res.changes > 0) {
      console.log(`[db migration] Backfilled developerId for ${res.changes} repositories.`)
    }
  }
} catch (err) {
  console.error('[db migration] Failed to backfill developerId:', err)
}
